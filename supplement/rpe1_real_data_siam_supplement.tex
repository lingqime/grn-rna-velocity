...
The recovered preprocessing notebook records two explicit perturbation-level filters. First, perturbation targets represented by fewer than 150 cells are removed, reducing the 985 measured perturbation targets to 286. Second, targets are required to have non-targeting mean spliced expression at least \(0.1\) and mean unspliced expression at least \(0.02\), reducing the panel to 165 perturbations. The notebook subsequently restores a 153-perturbation intermediate panel from a saved analysis checkpoint; the exact earlier filtering step responsible for the reduction from 165 to 153 perturbations is not represented in the recovered notebook and is therefore not reconstructed here. The final VeloCycle phase-coverage filter removes two additional perturbations, \texttt{NPLOC4} and \texttt{TRAPPC11}, leaving the 151-target panel used in the reported analysis.
...
For each condition \(q\), phase bin \(b\), and response gene \(g\), let \(\overline S_{g,b}^{(q)}\) denote the mean raw spliced count among cells in that condition--phase bin. With pseudocount \(0.1\), the condition-specific amplitude shift is
\[
\delta_g^{(q)}
=
\operatorname{median}_{b\in\mathcal B_q}
\left\{
\log\!\left(\overline S_{g,b}^{(q)}+0.1\right)
-
\log\!\left(\overline S_{g,b}^{(\mathrm{NT})}+0.1\right)
\right\},
\]
where \(\mathcal B_q\) contains bins having at least five cells in both condition \(q\) and the non-targeting control. The final spliced response trajectory is the common VeloCycle one-harmonic phase template multiplied by \(\exp(\delta_g^{(q)})\). This makes the condition dependence an NT-relative raw-spliced amplitude modulation of the VeloCycle phase shape.
...
The recovered calibration code uses the fixed 31-gene panel listed in the accompanying repository configuration. Candidate penalties are \(\lambda\in\operatorname{logspace}(-4,0,40)\). For each calibration gene, perturbation conditions are split into five folds with shuffling and random seed 2026, while the non-targeting condition is always retained in training. For every candidate \(\lambda\), the exact-integral model is fit using training-only FWL projection and RMS scaling, and held-out error is measured relative to the basal-only integral model. The selected global penalty minimizes the mean relative held-out mean-squared error across the 31 calibration genes, with equal weight per gene. The historical rule by which these 31 genes were originally chosen as the fixed calibration panel is not represented in the recovered notebook; their identities, however, are exactly preserved.
